...
const findOrCreateNode = (ID) => {
  ID = mermaid_ad1dc92f.e.sanitizeText(ID, (0,mermaid_ad1dc92f.c)());
  if (!nodesMap[ID]) {
...
const db = {
  nodesMap,
  getConfig: () => (0,mermaid_ad1dc92f.c)().sankey,
  getNodes,
  getLinks,
  getGraph,
  addLink,
...
const draw = function(text, id, _version, diagObj) {
  const { securityLevel, sankey: conf } = (0,mermaid_ad1dc92f.c)();
  const defaultSankeyConfig = mermaid_ad1dc92f.K.sankey;
  let sandboxElement;
...
${prefix}${Math.round(value * 100) / 100}${suffix}`;
  ...
  (0,mermaid_ad1dc92f.o)(void 0, svg, 0, useMaxWidth);
};
const renderer = {
  draw
};
